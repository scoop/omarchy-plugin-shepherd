...
Process {
    ...
    signal finishedWith(string text, int code, bool tooLarge)

    /** The last of the child's diagnostics, bounded like everything else. */
    property string stderrTail: ""

    /**
     * A single line handed to the child on stdin, then closed.
     *
     * This is how a helper is told what to do. Not argv: /proc/<pid>/cmdline is
     * readable by every process this user runs, so anything that must not be
     * published — and anything an attacker could learn something from, like
     * which host is being polled — travels here instead.
     */
    ...
    onExited: function (exitCode) {
        killTimer.stop();
        watchdog.stop();
        root.finishedWith(root._collected, exitCode, root._overflowed);
        root._collected = "";
        root._used = 0;
